...
uint8_t has_interrupted(struct pci_device_header_0 *device) {
    ...
    // If bit 3 of the status register == 1 and the bit 10 of the command
    // register == 0 return 1
...
const char *get_prog_interface(uint8_t class_code, uint8_t subclass_code,
...
void trigger_pci_interrupt() {
    // Iterate pci_dev_list_head and check if any of the devices have an
    // interrupt. If so call cb
    struct pci_dev_list *current = pci_dev_list_head;
    while (current != 0) {
        struct pci_device_header *device = current->device;
        if (current->device != 0 &&
            ((device->status & 0b1000) && !(device->command & 0b10000000000))) {
            if (current->cb != 0)
...
void subscribe_pci_interrupt(const char *id, struct pci_device_header *dev,
...
void register_pci_device(struct pci_device_header *pci,
                         uint32_t device_base_address,
                         char *(*cb)(void *, uint8_t, uint64_t)) {
    ...
    case 0x01: {
        ...
        case 0x06: {
            switch (pci->prog_if) {
            case 0x01:
                init_ahci(((struct pci_device_header_0 *)pci)
                              ->bar5); // TODO: Careful. This is a dynamic
                                       // driver startup.
...
void enumerate_function(uint64_t device_address, uint64_t bus, uint64_t device,
                        uint64_t function,
                        char *(*cb)(void *, uint8_t, uint64_t)) {
    ...
    struct pci_device_header *pci_device_header =
        (struct pci_device_header *)function_address;
    ...
    printf("[PCI] %s %s: %s %s / %s\n",
           ...
           get_subclass_name(pci_device_header->class_code,
           ...
           get_prog_interface(pci_device_header->class_code,
                              pci_device_header->subclass,
                              pci_device_header->prog_if));

    uint32_t device_base_address =
        ((1 << 31) | (bus << 16) | (device << 11) | (function << 8));
...
void enumerate_device(uint64_t bus_address, uint64_t device, uint64_t bus,
    ...
    struct pci_device_header *pci_device_header =
        (struct pci_device_header *)device_address;
...
void enumerate_bus(uint64_t base_address, uint64_t bus,
    ...
    // struct pci_device_header* pci_device_header = (struct
    // pci_device_header*)bus_address;

    // if (pci_device_header->device_id == 0x0) {printf("ig0:%ld|", bus);
    // return;} if (pci_device_header->device_id == 0xFFFF) {printf("igf:%ld|",
    // bus); return;}
...
void *get_bar_address(struct pci_device_header_0 *devh, uint8_t index) {
    ...
    case PCI_BAR_TYPE_64: {
        if (index == 5)
            return 0x0;
        return (void *)(uint64_t)((bar_value & 0xFFFFFFF0) |
                                  ((uint64_t)(bar_value2 & 0xFFFFFFFF) << 32));
    }
    default:
        break;
    }
...
uint64_t get_bar_size(void *addresslow,
                      uint32_t base_address) { // Address low is bar0.
    ...
    if (Type != PCI_BAR_TYPE_NULL) {
        ...
        *(uint32_t *)addresslow = BARValueLow;
        ...
        uint64_t Size =
            ((uint64_t)(SizeHigh & 0xFFFFFFFF) << 32) | (SizeLow & 0xFFFFFFFF);
        Size = ~Size + 1;
...
void register_pci(struct mcfg_header *mcfg,
                  char *(*cb)(void *, uint8_t, uint64_t)) {
    uint64_t entries = ((mcfg->header.length) - sizeof(struct mcfg_header)) /
                       sizeof(struct device_config);

    printf("[PCI] Registering pci devices... %ld entries\n", entries);
    for (uint64_t i = 0; i < entries; i++) {
        struct device_config *new_device_config =
            (struct device_config *)((uint64_t)mcfg +
                                     sizeof(struct mcfg_header) +
                                     (sizeof(struct device_config) * i));
        printf("[PCI] Base address as registering: %x\n",
               new_device_config->base_address);
        printf("[PCI] Enumerating buses %x:%x\n", new_device_config->start_bus,
               new_device_config->end_bus);
        for (uint64_t bus = new_device_config->start_bus;
             bus < new_device_config->end_bus; bus++) {
