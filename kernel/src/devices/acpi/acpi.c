...
const char acpi_signatures[][4] = {
    BERT_SIGNATURE, BOOT_SIGNATURE, BGRT_SIGNATURE, CPEP_SIGNATURE,
    CSRT_SIGNATURE, DBG2_SIGNATURE, DBGP_SIGNATURE, DSDT_SIGNATURE,
    DMAR_SIGNATURE, DRTM_SIGNATURE, ECDT_SIGNATURE, EINJ_SIGNATURE,
    ERST_SIGNATURE, ETDT_SIGNATURE, FACS_SIGNATURE, FADT_SIGNATURE,
    FPDT_SIGNATURE, GTDT_SIGNATURE, HEST_SIGNATURE, HPET_SIGNATURE,
    IBFT_SIGNATURE, IORT_SIGNATURE, IVRS_SIGNATURE, LPIT_SIGNATURE,
    MADT_SIGNATURE, MCFG_SIGNATURE, MCHI_SIGNATURE, MPST_SIGNATURE,
    MSCT_SIGNATURE, MSDM_SIGNATURE, NFIT_SIGNATURE, OEMx_SIGNATURE,
    PCCT_SIGNATURE, PMTT_SIGNATURE, PSDT_SIGNATURE, RASF_SIGNATURE,
    SBST_SIGNATURE, SLIC_SIGNATURE, SLIT_SIGNATURE, SPCR_SIGNATURE,
    SPMI_SIGNATURE, SRAT_SIGNATURE, SSDT_SIGNATURE, STAO_SIGNATURE,
    TCPA_SIGNATURE, TPM2_SIGNATURE, UEFI_SIGNATURE, WAET_SIGNATURE,
    WDAT_SIGNATURE, WDRT_SIGNATURE, WPBT_SIGNATURE, XENV_SIGNATURE,
    XSDT_SIGNATURE};
...
struct acpi_sdt_header *find_rsdt(struct rsdt *rsdt, const char *signature,
                                  uint64_t sig_len) {
    uint32_t entries = (rsdt->header.length - sizeof(struct acpi_sdt_header)) /
                       sizeof(uint32_t);
    printf("Navigating %d entries searching for %s\n", entries, signature);

    for (uint32_t i = 0; i < entries; i++) {
        uint32_t pto = rsdt->pointer_other_sdt[i];
        struct acpi_sdt_header *header = (struct acpi_sdt_header *)(uint64_t)
            pto; // TODO: delete this inmediately
...
struct acpi_sdt_header *find_xsdt(struct xsdt *xsdt, const char *signature,
                                  uint64_t sig_len) {
    uint32_t entries = (xsdt->header.length - sizeof(struct acpi_sdt_header)) /
                       sizeof(uint64_t);
    printf("Navigating %d entries searching for %s\n", entries, signature);
...
void *init_acpi_vt(void *rsdp_address, const char *target) {
    ...
    /*
    printf("RSDPv2 at: 0x%llx Sig: %.4s Check: %d OEM: %s Rev: %d RsdtAddr:
    0x%llx Len: 0x%llx XsdtAddr: 0x%llx ExtCheck: %d Res: %s\n", rsdp,
    signature, rsdp->first_part.checksum, oemid, rsdp->first_part.revision,
    rsdp->first_part.rsdt_address, rsdp->length, rsdp->xsdt_address,
    rsdp->extended_checksum, rsdp->reserved
    );
    */
...
void *init_acpi_vz(void *rsdp_address, const char *target) {
    ...
    /*
    printf("RSDP at: 0x%llx Sig: %.4s Check: %d OEM: %s Rev: %d RsdtAddr:
    0x%llx\n", rsdp, signature, rsdp->checksum, oemid, rsdp->revision,
    rsdp->rsdt_address
    );
    */
...
struct mcfg_header *get_acpi_mcfg() {
    ...
    printf("MCFG at: 0x%llx Sig: %.4s Len: 0x%llx Rev: %d Check: %d OEM: %s "
           "OTI: %s OR: %d CID: %d CREV: %d\n",
           mcfg_header, signature, mcfg_header->header.length,
           mcfg_header->header.revision, mcfg_header->header.checksum, oemid,
           oemtableid, mcfg_header->header.oem_revision,
...
struct madt_header *get_acpi_madt() {
    ...
    printf("MADT at: 0x%llx Sig: %.4s Len: 0x%llx Rev: %d Check: %d OEM: %s "
           "OTI: %s OR: %d CID: %d CREV: %d\n",
           madt_header, signature, madt_header->header.length,
           madt_header->header.revision, madt_header->header.checksum, oemid,
           oemtableid, madt_header->header.oem_revision,
           madt_header->header.creator_id,
           madt_header->header.creator_revision);
...
struct fadt_header *get_acpi_fadt() {
    ...
    printf("FADT at: 0x%llx Sig: %.4s Len: 0x%llx Rev: %d Check: %d OEM: %s "
           "OTI: %s OR: %d CID: %d CREV: %d\n",
           fadt_header, signature, fadt_header->header.length,
           fadt_header->header.revision, fadt_header->header.checksum, oemid,
           oemtableid, fadt_header->header.oem_revision,
           fadt_header->header.creator_id,
           fadt_header->header.creator_revision);
...
uint8_t is_enabled(struct fadt_header *fadt_header) {
    ...
    if (fadt_header->smi_command_port != 0)
        return 0;
    if (fadt_header->acpi_enable != 0 || fadt_header->acpi_disable != 0)
        return 0;
    if (!(fadt_header->pm1a_event_block & 1))
        return 0;
