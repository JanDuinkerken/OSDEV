...
char translate(uint8_t scancode, uint8_t uppercase) {
    if (scancode > 58)
        return 0;
    if (uppercase)
        return keyboard->ASCII_table[scancode] - 32;
...
void handle_keyboard(uint8_t scancode) {
    ...
    case Backspace:
        printf("\b \b");
        if (keyboard_callback != 0)
            keyboard_callback(Backspace);
        return;
    }

    char ascii = translate(scancode, keyboard->left_shift_pressed ||
                                         keyboard->right_shift_pressed);
    if (ascii != 0) {
        if (keyboard_callback != 0)
            keyboard_callback(ascii);
        printf("%c", ascii);
    }
}
