...
void driver_register_char(uint8_t major, const char *name,
...
void driver_register_block(uint8_t major, const char *name,
                           struct file_operations *fops) {
...
void device_list() {
    ...
    while (device->next != 0) {
        printf("Device: %s [MAJ: %x MIN: %x ID: %x CTRL: %x]\n", device->name,
               device->bc << 7 | device->major, device->minor,
               device->internal_id, device->device_control_structure);
        device = device->next;
    }
}
...
char *device_create(void *device_control_structure, uint8_t major,
                    uint64_t id) {
    ...
    printf("Registering device: %s [MAJ: %x MIN: %x ID: %x CTRL: %x]\n", name,
           major, minor, id, device_control_structure);
...
void device_destroy(char *device_name) {
    ...
    while (device->valid) {
        if (memcmp((void *)device->name, (void *)device, strlen(device_name)) ==
            0) {
            device->valid = 0;
            if (prev != 0) {
...
char *insert_device_cb(void *device_control_structure, uint8_t major,
                       uint64_t id) {
        ...
        if (memcmp((void *)dev->name, (void *)device, strlen(device)) == 0) {
            if (dev->bc == 0) {
                if (!block_device_drivers[dev->major].registered)
                    return 0;
                return !strcmp(block_device_drivers[dev->major].name,
                               driver_name);
            } else {
                if (!char_device_drivers[dev->major].registered)
                    return 0;
                return !strcmp(char_device_drivers[dev->major].name,
                               driver_name);
            }
        }
        dev = dev->next;
...
uint64_t device_write(const char *name, uint64_t size, uint64_t offset,
                      uint8_t *buffer) {
    struct device *dev = device_header;
    while (dev->valid) {
        if (memcmp((void *)dev->name, (void *)name, strlen(name)) == 0) {
            struct device_driver driver = (dev->bc == 0)
                                              ? block_device_drivers[dev->major]
                                              : char_device_drivers[dev->major];
...
uint64_t device_read(const char *device, uint64_t size, uint64_t offset,
                     uint8_t *buffer) {
    struct device *dev = device_header;
    while (dev->valid) {
        if (memcmp((void *)dev->name, (void *)device, strlen(device)) == 0) {
