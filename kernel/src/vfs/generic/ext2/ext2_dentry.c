...
uint8_t ext2_delete_dentry(struct ext2_partition *partition, const char *path) {
    ...
    EXT2_DEBUG("Deleting file %s, inode: %d parent_inode: %d", path,
               target_inode_number, parent_inode_number);
    uint8_t deleted = 0;
    if (parent_inode->i_mode & INODE_TYPE_DIR) {
        ...
        if (ext2_read_inode_bytes(partition, parent_inode_number, block_buffer,
                                  parent_inode->i_size,
                                  0) == EXT2_READ_FAILED) {
            EXT2_ERROR("Failed to read parent directory %s", parent_path);
            return 1;
        ...
        if (deleted)
            if (ext2_write_inode_bytes(partition, parent_inode_number,
                                       block_buffer, parent_inode->i_size,
                                       0) == EXT2_WRITE_FAILED) {
...
uint8_t ext2_dentry_get_dentry(struct ext2_partition *partition,
    ...
    if (root_inode->i_mode & INODE_TYPE_DIR) {
        ...
        if (ext2_read_inode_bytes(partition, inode_number, block_buffer,
...
void ext2_list_dentry(struct ext2_partition *partition, const char *path) {
    ...
    if (root_inode->i_mode & INODE_TYPE_DIR) {
        ...
        while (parsed_bytes < root_inode->i_size && list_count < LIST_MAX) {
            struct ext2_directory_entry *entry =
                (struct ext2_directory_entry *)(block_buffer + parsed_bytes);
            printf("[EXT2] ino: %d rec_len: %d name_len: %d file_type: %d "
                   "name: %s\n",
...
uint32_t ext2_get_all_dirs(struct ext2_partition *partition,
...
uint8_t
ext2_operate_on_dentry(struct ext2_partition *partition, const char *path,
                       uint8_t (*callback)(struct ext2_partition *partition,
...
uint8_t ext2_initialize_directory(struct ext2_partition *partition,
    ...
    uint32_t entry_size_first = sizeof(struct ext2_directory_entry) +
                                entries[0].name_len - EXT2_NAME_LEN;
    entries[0].rec_len = (entry_size_first + 3) & ~3;

    uint32_t entry_size_second = sizeof(struct ext2_directory_entry) +
                                 entries[1].name_len - EXT2_NAME_LEN;
    ...
    if (memcpy(block_buffer + entries[0].rec_len, &entries[1],
               entry_size_second) == 0) {
    ...
    if (ext2_write_inode_bytes(partition, inode_number, block_buffer,
                               root_inode->i_size, 0) == EXT2_WRITE_FAILED) {
...
uint8_t ext2_create_directory_entry(struct ext2_partition *partition,
                                    uint32_t inode_number, uint32_t child_inode,
                                    const char *name, uint32_t type) {
    EXT2_DEBUG("File name: %s, Parent inode: %d, Type: %d", name, inode_number,
               type);
    ...
    struct ext2_directory_entry child_entry = {.inode = child_inode,
                                               .rec_len = 0,
                                               .name_len = strlen(name),
                                               .file_type = type};

    uint32_t entry_size = sizeof(struct ext2_directory_entry) +
                          child_entry.name_len - EXT2_NAME_LEN;
    ...
    if (root_inode->i_mode & INODE_TYPE_DIR) {
        ...
        if (memcpy(block_buffer + parsed_bytes - original_rec_len, entry,
                   entry_size) == 0) {
        ...
        if (memcpy(block_buffer + parsed_bytes - original_rec_len +
                       entry_size_aligned,
                   &child_entry,
                   sizeof(struct ext2_directory_entry) - EXT2_NAME_LEN) == 0) {
            EXT2_ERROR("Failed to copy directory entry");
            free(block_buffer);
            return 1;
        }
        ...
        if (memcpy(block_buffer + parsed_bytes - original_rec_len +
                       entry_size_aligned +
                       sizeof(struct ext2_directory_entry) - EXT2_NAME_LEN,
                   name, child_entry.name_len) == 0) {
            EXT2_ERROR("Failed to copy directory entry");
            free(block_buffer);
            return 1;
        }
