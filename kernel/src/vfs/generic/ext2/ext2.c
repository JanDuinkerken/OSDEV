...
// Supress  warning: ISO C99 requires at least one argument for the "..." in a
// variadic macro
...
char *ext2_file_type_names[8] = {"unknown",  "regular", "directory", "chardev",
                                 "blockdev", "fifo",    "socket",    "symlink"};
...
#define EXT2_TRANSLATE_DENTRY_TO_NATIVE(dentry)                                \
    ({                                                                         \
        uint32_t i = 0;                                                        \
        for (; i < 8; i++) {                                                   \
            if (file_type_translator[i][EXT2_DENTRY_TRANSLATOR_INDEX] ==       \
                dentry) {                                                      \
                break;                                                         \
            }                                                                  \
        }                                                                      \
...
#define EXT2_TRANSLATE_INODE_TO_NATIVE(inode)                                  \
    ({                                                                         \
        uint32_t i = 0;                                                        \
        for (; i < 8; i++) {                                                   \
            if (file_type_translator[i][EXT2_INODE_TRANSLATOR_INDEX] ==        \
                inode) {                                                       \
                break;                                                         \
            }                                                                  \
        }                                                                      \
...
#define EXT2_TRANSLATE_NATIVE_TO_DENTRY(native)                                \
    (EXT2_TRANSLATE_UNIT(native, EXT2_DENTRY_TRANSLATOR_INDEX))
#define EXT2_TRANSLATE_NATIVE_TO_INODE(native)                                 \
    (EXT2_TRANSLATE_UNIT(native, EXT2_INODE_TRANSLATOR_INDEX))
...
uint32_t ext2_count_partitions() { return ext2_partition_count_partitions(); }
...
uint8_t split_into_path_and_name(const char *full_path, char *parent,
                                 char *name) {
...
uint64_t ext2_get_file_size(struct ext2_partition *partition,
                            const char *path) {
...
uint8_t ext2_list_directory(struct ext2_partition *partition,
                            const char *path) {
...
uint8_t ext2_get_dentry(struct ext2_partition *partition, const char *path,
...
uint8_t ext2_read_directory(struct ext2_partition *partition, const char *path,
                            uint32_t *count,
                            struct ext2_directory_entry **buffer) {
...
uint8_t ext2_create_file(struct ext2_partition *partition, const char *path,
    ...
    struct ext2_inode_descriptor_generic *parent_inode =
        (struct ext2_inode_descriptor_generic *)ext2_read_inode(
            partition, parent_inode_index);
    if (parent_inode == 0) {
    ...
    if (!(parent_inode->i_mode & INODE_TYPE_DIR)) {
        EXT2_WARN("Parent inode for path %s is not a directory: %d",
                  parent_path, parent_inode->i_mode);
        return EXT2_RESULT_ERROR;
    }
    ...
    struct ext2_inode_descriptor *inode = ext2_initialize_inode(
        partition, new_inode_index, EXT2_TRANSLATE_NATIVE_TO_INODE(type),
        permissions);
    ...
    if (type == EXT2_FILE_TYPE_DIRECTORY) {
        ...
        if (ext2_resize_file(partition, new_inode_index, block_size) !=
            EXT2_RESULT_OK) {
        ...
        if (ext2_initialize_directory(partition, new_inode_index,
                                      parent_inode_index)) {
    ...
    if (ext2_create_directory_entry(partition, parent_inode_index,
                                    new_inode_index, name,
                                    EXT2_TRANSLATE_NATIVE_TO_DENTRY(type))) {
...
uint8_t ext2_resize_file(struct ext2_partition *partition, uint32_t inode_index,
                         uint32_t new_size) {
    ...
    if (inode->i_size > new_size) {
        ...
        EXT2_DEBUG("Resizing file to %d bytes, deallocating %d blocks",
                   new_size, blocks_to_deallocate);
        ...
        if (ext2_deallocate_blocks(partition, blocks, blocks_to_deallocate) !=
        ...
        if ((new_size - inode->i_size) % block_size)
            blocks_to_allocate++;

        EXT2_DEBUG("Resizing file to %d bytes, allocating %d blocks", new_size,
                   blocks_to_allocate);
    ...
    if (ext2_write_inode(partition, inode_index,
...
uint8_t ext2_read_file(struct ext2_partition *partition, const char *path,
                       uint8_t *destination_buffer, uint64_t size,
                       uint64_t skip) {
    ...
    if ((skip + size) > inode->i_size) {
        EXT2_WARN("Trying to read past the end of the file[skip=%d, size=%d, "
                  "file_size=%d]",
                  skip, size, inode->i_size);
        return EXT2_RESULT_ERROR;
    }

    int64_t read_bytes = ext2_read_inode_bytes(partition, inode_index,
                                               destination_buffer, size, skip);
...
uint32_t ext2_get_inode_index(struct ext2_partition *partition,
                              const char *path) {
...
uint8_t ext2_write_file(struct ext2_partition *partition, const char *path,
                        uint8_t *source_buffer, uint64_t size, uint64_t skip) {
    ...
    int64_t write_bytes = ext2_write_inode_bytes(partition, inode_index,
                                                 source_buffer, size, skip);
...
uint8_t ext2_debug(struct ext2_partition *partition) {
    ...
    ext2_dump_all_inodes(partition,
                         "/"); // TODO: Get the root inode name from somewhere
    return EXT2_RESULT_OK;
}
...
uint8_t ext2_errors() { return ext2_has_errors(EXT2_ERROR_ERROR); }

uint16_t ext2_get_file_permissions(struct ext2_partition *partition,
                                   const char *path) {
    uint32_t inode_index = ext2_path_to_inode(partition, path);
    if (!inode_index) {
        EXT2_WARN("File doesn't exist");
