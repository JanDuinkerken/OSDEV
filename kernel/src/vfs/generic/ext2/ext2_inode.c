...
void ext2_dump_inode_bitmap(struct ext2_partition *partition) {
    ...
    for (uint32_t i = 0; i < block_group_count; i++) {
        struct ext2_block_group_descriptor *bgd =
            (struct ext2_block_group_descriptor *)&partition->gd[i];
        uint8_t *bitmap =
            (uint8_t *)ext2_buffer_for_size(block_size, block_size);
        if (bitmap == 0) {
...
uint32_t ext2_allocate_inode(struct ext2_partition *partition) {
    ...
    int32_t ext2_block_group_id =
        ext2_operate_on_bg(partition, ext2_bg_has_free_inodes);
    ...
    struct ext2_block_group_descriptor *bgd =
        (struct ext2_block_group_descriptor *)&partition
            ->gd[ext2_block_group_id];
    ...
    struct ext2_superblock_extended *sb =
        (struct ext2_superblock_extended *)partition->sb;
...
void ext2_debug_print_file_inode(struct ext2_partition *partition,
    ...
    struct ext2_inode_descriptor *inode =
        ext2_read_inode(partition, inode_number);
    if (inode == 0) {
...
ext2_initialize_inode(struct ext2_partition *partition, uint32_t inode_number,
                      uint32_t type, uint32_t permissions) {
    struct ext2_inode_descriptor *inode_descriptor =
        ext2_read_inode(partition, inode_number);
...
uint8_t *ext2_read_inode_bitmap(struct ext2_partition *partition,
                                uint32_t inode_number) {
    ...
    uint32_t inode_bitmap_lba =
        (inode_bitmap_block * block_size) / partition->sector_size;
    ...
    if (!read_disk(partition->disk, inode_bitmap_buffer,
                   partition->lba + inode_bitmap_lba, sectors_per_block)) {
...
uint8_t ext2_write_inode_bitmap(struct ext2_partition *partition,
                                uint32_t inode_number, uint8_t *inode_bitmap) {
    struct ext2_superblock *superblock =
        (struct ext2_superblock *)partition->sb;
    uint32_t block_size = 1024 << superblock->s_log_block_size;
    uint32_t inode_group = (inode_number - 1) / superblock->s_inodes_per_group;
    ...
    uint32_t inode_bitmap_lba =
        (inode_bitmap_block * block_size) / partition->sector_size;
    if (!write_disk(partition->disk, inode_bitmap,
                    partition->lba + inode_bitmap_lba, sectors_per_block)) {
...
uint8_t ext2_write_inode(struct ext2_partition *partition,
                         uint32_t inode_number,
                         struct ext2_inode_descriptor *inode) {
    struct ext2_superblock *superblock =
        (struct ext2_superblock *)partition->sb;
    struct ext2_superblock_extended *superblock_extended =
        (struct ext2_superblock_extended *)partition->sb;
    ...
    uint32_t inode_size =
        (superblock->s_rev_level < 1) ? 128 : superblock_extended->s_inode_size;
    ...
    uint32_t inode_table_lba =
        (inode_table_block * block_size) / partition->sector_size;
    ...
    if (!read_disk(partition->disk, root_inode_buffer,
                   partition->lba + inode_table_lba +
                       inode_block * sectors_per_block,
                   sectors_per_block)) {
    ...
    if (!write_disk(partition->disk, root_inode_buffer,
                    partition->lba + inode_table_lba +
                        inode_block * sectors_per_block,
                    sectors_per_block)) {
...
struct ext2_inode_descriptor *ext2_read_inode(struct ext2_partition *partition,
                                              uint32_t inode_number) {
    ...
    if (memcpy(inode,
               root_inode_buffer + (((inode_index * inode_size) % block_size)),
               inode_size) == 0) {
        EXT2_ERROR("Failed to copy root inode");
        free(root_inode_buffer);
        free(inode);
...
uint32_t ext2_inode_from_path_and_parent(struct ext2_partition *partition,
                                         uint32_t parent_inode,
                                         const char *path) {
    uint32_t block_size =
        1024 << (((struct ext2_superblock *)partition->sb)->s_log_block_size);
    struct ext2_inode_descriptor_generic *root_inode =
    ...
    if (root_inode->i_mode & INODE_TYPE_DIR) {
        ...
        if (ext2_read_inode_bytes(partition, parent_inode, block_buffer,
                                  root_inode->i_size, 0) == EXT2_READ_FAILED) {
        ...
        while (parsed_bytes < root_inode->i_size) {
            struct ext2_directory_entry *entry =
                (struct ext2_directory_entry *)(block_buffer + parsed_bytes);
...
uint32_t ext2_path_to_inode(struct ext2_partition *partition,
                            const char *path) {
    ...
    while (token != 0) {
        inode_index =
            ext2_inode_from_path_and_parent(partition, inode_index, token);
...
uint32_t ext2_load_indirect_block_list(struct ext2_partition *partition,
                                       uint32_t *block_buffer, uint32_t block) {
    uint32_t block_size =
        1024 << (((struct ext2_superblock *)partition->sb)->s_log_block_size);
    ...
    if (ext2_read_block(partition, block, (uint8_t *)indirect_block) ==
        EXT2_READ_FAILED) {
...
uint32_t ext2_load_double_indirect_block_list(struct ext2_partition *partition,
    ...
    for (uint32_t i = 0; i < block_size / 4; i++) {
        ...
        uint32_t read = ext2_load_indirect_block_list(
            partition, block_buffer + blocks_read, indirect_block[i]);
...
uint32_t ext2_load_triple_indirect_block_list(struct ext2_partition *partition,
                                              uint32_t *block_buffer,
                                              uint32_t block) {
    uint32_t block_size =
        1024 << (((struct ext2_superblock *)partition->sb)->s_log_block_size);
    ...
    for (uint32_t i = 0; i < block_size / 4; i++) {
        ...
        uint32_t read = ext2_load_double_indirect_block_list(
            partition, block_buffer + blocks_read, indirect_block[i]);
        if (read == EXT2_READ_FAILED) {
            EXT2_ERROR("Failed to read indirect block");
            free(indirect_block);
...
uint32_t *ext2_load_block_list(struct ext2_partition *partition,
                               uint32_t inode_number) {
    uint32_t block_size =
        1024 << (((struct ext2_superblock *)partition->sb)->s_log_block_size);
    ...
    if (block_index < block_number) {
        ...
        uint32_t read = ext2_load_indirect_block_list(
            partition, block_list + block_index, inode->i_block[12]);
        if (read == EXT2_READ_FAILED) {
            EXT2_ERROR("Failed to read indirect block");
            free(block_list);
            return 0;
    ...
    if (block_index < block_number) {
        EXT2_DEBUG("[%d/%d] Reading double indirect block list", block_index,
                   block_number);
        uint32_t read = ext2_load_double_indirect_block_list(
            partition, block_list + block_index, inode->i_block[13]);
        if (read == EXT2_READ_FAILED) {
            EXT2_ERROR("Failed to read indirect block");
            free(block_list);
            return 0;
    ...
    if (block_index < block_number) {
        ...
        uint32_t read = ext2_load_triple_indirect_block_list(
            partition, block_list + block_index, inode->i_block[14]);
        if (read == EXT2_READ_FAILED) {
            EXT2_ERROR("Failed to read indirect block");
            free(block_list);
            return 0;
    ...
    if (block_index != block_number) {
        EXT2_ERROR("Block list size mismatch! Expected %d, got %d",
...
uint8_t ext2_delete_file_blocks(struct ext2_partition *partition,
                                uint32_t inode_number) {
    uint32_t block_size =
        1024 << (((struct ext2_superblock *)partition->sb)->s_log_block_size);
    struct ext2_inode_descriptor_generic *inode =
    ...
    EXT2_DEBUG("Deallocating %d blocks for inode %d", block_number,
               inode_number);

    if (ext2_deallocate_blocks(partition, block_list, block_number) !=
        block_number) {
...
uint8_t ext2_delete_n_blocks(struct ext2_partition *partition,
                             uint32_t inode_number, uint32_t blocks_to_remove) {
    struct ext2_inode_descriptor_generic *inode =
        (struct ext2_inode_descriptor_generic *)ext2_read_inode(partition,
                                                                inode_number);
    if (inode == 0) {
    ...
    EXT2_DEBUG("Deallocating %d blocks for inode %d", blocks_to_remove,
               inode_number);
    if (ext2_deallocate_blocks(partition, block_list, blocks_to_remove) !=
        blocks_to_remove) {
        EXT2_ERROR("Failed to deallocate blocks for inode %d", inode_number);
        free(block_list);
        return 1;
...
uint8_t ext2_delete_inode(struct ext2_partition *partition,
                          uint32_t inode_number) {
    struct ext2_inode_descriptor *full_inode =
        ext2_read_inode(partition, inode_number);
    if (full_inode == 0) {
        EXT2_ERROR("Failed to read inode %d", inode_number);
        return 1;
    ...
    uint8_t *inode_bitmap_buffer =
        ext2_read_inode_bitmap(partition, inode_number);
    if (inode_bitmap_buffer == 0) {
...
void ext2_dump_inode(struct ext2_inode_descriptor_generic *inode) {
    ...
    printf("i_atime: %x i_ctime: %x i_mtime: %x i_dtime: %x\n", inode->i_atime,
           inode->i_ctime, inode->i_mtime, inode->i_dtime);
    printf("i_gid: %x i_links_count: %x i_sectors: %x i_flags: %x i_osd1: %x\n",
           inode->i_gid, inode->i_links_count, inode->i_sectors, inode->i_flags,
           inode->i_osd1);
    printf("i_generation: %x i_file_acl: %x i_dir_acl: %x i_faddr: %x\n",
           inode->i_generation, inode->i_file_acl, inode->i_dir_acl,
           inode->i_faddr);
...
uint8_t ext2_dentry_walk_inodes_cb(struct ext2_partition *partition,
...
void ext2_dump_all_inodes(struct ext2_partition *partition,
    ...
    for (uint32_t i = 0; i < entry_count; i++) {
        ...
        if (entry->file_type == EXT2_DIR_TYPE_DIRECTORY) {
            ...
            char *new_path =
                malloc(strlen(root_path) + strlen(entry->name) + 2);
            if (new_path == 0) {
