...
int ext2_compat_register_partition(const char *drive, uint32_t lba,
    ...
    for (int i = 0; i < MAX_EXT2_PARTITIONS; i++) {
        if (ext2_partitions[i] == 0) {
            ext2_partitions[i] =
                ext2_register_partition(drive, lba, mountpoint);
            if (ext2_partitions[i] == 0) {
                return -1;
            }
            return i;
...
int ext2_compat_file_creat(int partno, const char *path, int mode) {
    ...
    // uint8_t ext2_create_file(struct ext2_partition * partition, const char*
    // path, uint32_t type, uint32_t permissions);
    uint8_t res =
        ext2_create_file(partition, path, EXT2_FILE_TYPE_REGULAR, mode);
...
int ext2_compat_dir_creat(int partno, const char *path, int mode) {
    ...
    uint8_t res =
        ext2_create_file(partition, path, EXT2_FILE_TYPE_DIRECTORY, mode);
    if (res != EXT2_RESULT_OK) {
        return -1;
    }
...
uint64_t ext2_compat_file_read(int partno, int fd, void *buffer,
                               uint64_t size) {
    if (partno < 0 || partno >= MAX_EXT2_PARTITIONS)
        return 0;
    struct ext2_partition *partition = ext2_partitions[partno];
...
int ext2_compat_dir_read(int partno, int fd, char *name, uint32_t *name_len,
                         uint32_t *type) {
    (void)partno;
...
int ext2_compat_dir_load(int partno, int fd) {
    ...
    uint32_t count = 0;
    uint8_t result =
        ext2_read_directory(partition, entry->name, &count, &entries);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        add_file_to_dirfd(fd, entry->name, entry->inode, entry->file_type,
                          entry->name_len);
    }
...
uint64_t ext2_compat_file_seek(int partno, int fd, uint64_t offset,
                               int whence) {
    if (partno < 0 || partno >= MAX_EXT2_PARTITIONS)
        return 0;
    struct ext2_partition *partition = ext2_partitions[partno];
...
uint64_t ext2_compat_file_write(int partno, int fd, void *buffer,
                                uint64_t size) {
    if (partno < 0 || partno >= MAX_EXT2_PARTITIONS)
        return -1;
    struct ext2_partition *partition = ext2_partitions[partno];
    ...
    struct file_descriptor_entry *entry = vfs_compat_get_file_descriptor(fd);
    uint64_t written =
        ext2_write_file(partition, entry->name, buffer, size, entry->offset);
...
int ext2_compat_rename(int partno, const char *path, const char *newpath) {
    (void)partno;
    (void)path;
    (void)newpath;
    return -1;
...
int ext2_compat_chmod(int partno, const char *path, int mode) {
    (void)partno;
    (void)path;
    (void)mode;
    return -1;
