...
#define EXT2_ERROR(error, ...)                                                 \
    ext2_add_error(ext2_format_message(error, ##__VA_ARGS__), __func__,        \
                   __FILE__, __LINE__, EXT2_ERROR_ERROR)
#define EXT2_WARN(error, ...)                                                  \
    ext2_add_error(ext2_format_message(error, ##__VA_ARGS__), __func__,        \
                   __FILE__, __LINE__, EXT2_ERROR_WARN)
#define EXT2_INFO(error, ...)                                                  \
    ext2_add_error(ext2_format_message(error, ##__VA_ARGS__), __func__,        \
                   __FILE__, __LINE__, EXT2_ERROR_INFO)
#define EXT2_DEBUG(error, ...)                                                 \
    ext2_add_error(ext2_format_message(error, ##__VA_ARGS__), __func__,        \
                   __FILE__, __LINE__, EXT2_ERROR_DEBUG)
