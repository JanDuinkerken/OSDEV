...
struct ext2_partition *get_partition(struct ext2_partition *partition,
                                     const char *partno) {
    while (partition != 0) {
...
ext2_partition_register_partition(const char *disk, uint32_t lba,
                                  const char *mountpoint) {
    EXT2_INFO("Registering partition on disk %s at LBA %d, mountpoint: %s",
              disk, lba, mountpoint);
    ...
    struct ext2_superblock *superblock =
        (struct ext2_superblock *)superblock_buffer;
    ...
    EXT2_DEBUG("Superblock magic valid, ext2 version: %d",
    ...
    uint32_t block_groups_first = DIVIDE_ROUNDED_UP(
        superblock->s_blocks_count, superblock->s_blocks_per_group);
    uint32_t block_groups_second = DIVIDE_ROUNDED_UP(
    ...
    for (uint32_t i = 0; i < block_groups_first; i++) {
        if (!read_disk(disk, dummy_sb_buffer,
                       lba + (i * sectors_per_group) + SB_OFFSET_LBA, 2)) {
            EXT2_ERROR("Failed to read dummy superblock");
            return 0;
        }
        struct ext2_superblock *dummy_sb =
            (struct ext2_superblock *)dummy_sb_buffer;
    ...
    uint32_t block_group_descriptors_size = DIVIDE_ROUNDED_UP(
        block_groups_first * sizeof(struct ext2_block_group_descriptor),
        sector_size);
    EXT2_DEBUG("Block group descriptors size: %d",
               block_group_descriptors_size);
    // TODO: End of sanity check

    void *block_group_descriptor_buffer =
        malloc(block_group_descriptors_size * sector_size);

    if (!read_disk(disk, (uint8_t *)block_group_descriptor_buffer,
                   lba + (sectors_per_block * bgdt_block),
                   block_group_descriptors_size)) {
        EXT2_ERROR("Failed to read block group descriptor table");
        return 0;
    }
    struct ext2_block_group_descriptor *block_group_descriptor =
        (struct ext2_block_group_descriptor *)block_group_descriptor_buffer;
    ...
    memcpy(partition->gd, block_group_descriptor,
           block_group_descriptors_size * sector_size);

    EXT2_DEBUG("Partition %s has: %d groups", partition->name,
               block_groups_first);
