...
#define CTRL_SYNC                                                              \
    4 /* Complete pending write process (needed at FF_FS_READONLY == 0) */
#define GET_SECTOR_COUNT 7 /* Get media size (needed at FF_USE_MKFS == 1) */
#define GET_SECTOR_SIZE                                                        \
    6 /* Get sector size (needed at FF_MAX_SS != FF_MIN_SS) */
#define GET_BLOCK_SIZE 8 /* Get erase block size (needed at FF_USE_MKFS == 1)  \
                          */
#define CTRL_TRIM                                                              \
    5 /* Inform device that the data on the block of sectors is no longer used \
