...
int fifo_compat_register_device(const char *device, uint32_t mode,
...
uint64_t fifo_compat_file_write(int devno, int fd, void *buffer,
                                uint64_t size) {
    if (devno < 0 || devno >= MAX_FIFO_DEVICES)
        return -1;
...
int fifo_compat_flush(int partno) {
    (void)partno;
    return -1;
}
int fifo_compat_dir_open(int partno, const char *path) {
    (void)partno;
    (void)path;
    return -1;
}
int fifo_compat_dir_close(int partno, int fd) {
    (void)partno;
    (void)fd;
    return -1;
}
int fifo_compat_file_creat(int partno, const char *path, int mode) {
    (void)partno;
    (void)path;
    (void)mode;
    return -1;
}
int fifo_compat_dir_creat(int partno, const char *path, int mode) {
    (void)partno;
    (void)path;
    (void)mode;
    return -1;
}
int fifo_compat_dir_read(int partno, int fd, char *name, uint32_t *name_len,
                         uint32_t *type) {
    (void)partno;
    (void)fd;
    (void)name_len;
...
int fifo_compat_dir_load(int partno, int fd) {
    (void)partno;
    (void)fd;
    return -1;
}
int fifo_compat_stat(int partno, int fd, stat_t *st) {
    (void)partno;
    (void)fd;
    (void)st;
    return -1;
}
int fifo_compat_rename(int partno, const char *path, const char *newpath) {
    (void)partno;
    (void)path;
    (void)newpath;
    return -1;
}
int fifo_compat_prepare_remove(int partno, const char *path) {
    (void)partno;
    (void)path;
    return -1;
}
int fifo_compat_remove(int partno, const char *path) {
    (void)partno;
    (void)path;
    return -1;
}
int fifo_compat_chmod(int partno, const char *path, int mode) {
    (void)partno;
    (void)path;
    (void)mode;
    return -1;
