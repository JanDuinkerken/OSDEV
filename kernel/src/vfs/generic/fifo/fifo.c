...
struct vfs_fifo *fifo_register_device(const char *device, uint32_t mode,
                                      const char *mountpoint) {

    struct vfs_fifo *fifo =
        (struct vfs_fifo *)calloc(1, sizeof(struct vfs_fifo));
    if (!fifo)
        return 0;
...
uint8_t fifo_search(const char *name) { return (uint8_t)fifo_identify(name); }

int fifo_sync(struct vfs_fifo *fifo) { return fifo_dev_flush(fifo->device); }
...
uint8_t vfs_fifo_read(struct vfs_fifo *fifo, uint8_t *destination_buffer,
                      uint64_t size, uint64_t skip) {
    return fifo_read(fifo->device, destination_buffer, skip, size);
}
uint8_t vfs_fifo_write(struct vfs_fifo *fifo, uint8_t *source_buffer,
                       uint64_t size, uint64_t skip) {
