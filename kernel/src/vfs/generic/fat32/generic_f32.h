...
int f32_compat_register_partition(const char *path, uint32_t lba,
    ...
    for (int index = 0; index < MAX_FAT32_PARTITIONS; index++) {
        if (f32_compat_partitions[index] == 0) {
            f32_compat_partitions[index] =
                register_fat32_partition(path, lba, mountpoint);
            return index;
        }
    }
...
int f32_compat_file_open(int partno, const char *path, int flags, int mode) {
    printf("VFS REQUESTED FAT32 FILE OPEN [partno: %d path: %s flags: %d mode: "
           "%d]\n",
           partno, path, flags, mode);
    return -1;
}
int f32_compat_file_close(int partno, int fd) {
    (void)partno;
    (void)fd;
    return -1;
}
int f32_compat_file_creat(int partno, const char *path, int mode) {
    (void)partno;
    (void)path;
    (void)mode;
    return -1;
}
uint64_t f32_compat_file_read(int partno, int fd, void *buffer, uint64_t size) {
    (void)partno;
    (void)fd;
    (void)buffer;
    (void)size;
...
uint64_t f32_compat_file_write(int partno, int fd, void *buffer,
                               uint64_t size) {
    (void)partno;
    (void)fd;
    (void)buffer;
...
uint64_t f32_compat_file_seek(int partno, int fd, uint64_t offset, int whence) {
    (void)partno;
    (void)fd;
    (void)offset;
    (void)whence;
...
uint64_t f32_compat_file_tell(int partno, int fd) {
    (void)partno;
    (void)fd;
    return 1;
}
int f32_compat_stat(int partno, int fd, stat_t *st) {
    (void)partno;
    (void)fd;
    (void)st;
    return -1;
}
int f32_compat_dir_open(int partno, const char *path) {
    (void)partno;
    (void)path;
    return -1;
}
int f32_compat_dir_close(int partno, int dir) {
    (void)partno;
    (void)dir;
    return -1;
}
int f32_compat_dir_load(int partno, int dir) {
    (void)partno;
    (void)dir;
    return -1;
}
int f32_compat_dir_read(int partno, int dir, char *path, uint32_t *name_len,
                        uint32_t *type) {
    (void)partno;
    (void)path;
    (void)dir;
...
int f32_compat_dir_creat(int partno, const char *path, int mode) {
    (void)partno;
    (void)path;
    (void)mode;
    return -1;
}
int f32_compat_rename(int partno, const char *path, const char *newpath) {
    (void)partno;
    (void)path;
    (void)newpath;
    return -1;
}
int f32_compat_remove(int partno, const char *path) {
    (void)partno;
    (void)path;
    return -1;
}
int f32_compat_chmod(int partno, const char *path, int mode) {
    (void)partno;
    (void)path;
    (void)mode;
    return -1;
