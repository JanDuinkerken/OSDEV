...
uint8_t fat_table_set(struct volume_s *vol, uint32_t cluster,
                      uint32_t fat_entry);
...
uint8_t fat_search(const char *name, uint32_t lba) {
    uint8_t bpb[512];
    if (!disk_read(name, bpb, lba, 1))
        return 0;

    // Check the BPB boot signature
    if (load16(bpb + 510) != 0xAA55)
        return 0;
    ...
    uint32_t data_sectors =
        tot_sect - (load16(bpb + BPB_RSVD_CNT) +
...
uint8_t fat_dir_set_index(struct dir_s *dir, uint32_t index) { return 1; }
...
uint8_t fat_table_get(struct volume_s *vol, uint32_t cluster,
                      uint32_t *fat_entry) {
...
uint8_t fat_get_cluster(struct volume_s *vol, uint32_t *cluster) {
    ...
    store32(vol->buffer + INFO_NEXT_FREE,
        ...
        if (!((sfn_tmp == 0x00) || (sfn_tmp == 0x05) || (sfn_tmp == 0xE5))) {
            ...
            if ((buffer[rw_offset + SFN_ATTR] & ATTR_LFN) == ATTR_LFN) {
                ...
                if (lfn_crc && lfn_match) {
                    ...
                    if (fat_dir_sfn_cmp((char *)buffer + rw_offset, name,
                                        size)) {
                        match = 1;
                    }
                }
                if (match) {
                    ...
                    dir->cluster =
                        (load16(buffer + rw_offset + SFN_CLUSTH) << 16) |
                        load16(buffer + rw_offset + SFN_CLUSTL);
                    dir->sector = fat_clust_to_sect(dir->vol, dir->cluster);
...
void fat_print_info(struct info_s *info) {
    ...
    printf("%d/%d/%d %d:%d\t", date & 0x1f, (date >> 5) & 0xf,
           ((date >> 9) & 0x7f) + 1980, (time >> 11) & 0x1f,
           (time >> 5) & 0x3f);
...
char register_fat32_partition(const char *disk, uint32_t lba,
    ...
    vol->data_lba = vol->fat_lba + (load32(mount_buffer + BPB_32_FAT_SIZE) *
                                    mount_buffer[BPB_NUM_FATS]);

    vol->root_lba =
        fat_clust_to_sect(vol, load32(mount_buffer + BPB_32_ROOT_CLUST));
    ...
    for (uint8_t i = 0; i < valid_partitions; i++) {
        if (partitions[i].lba) {
            ...
            if (fat_search(mount_buffer)) {
                ...
                struct volume_s* vol = (struct volume_s *)malloc(sizeof(struct
volume_s)); memset(vol, 0, sizeof(struct volume_s));
...
/// NULL
struct volume_s *volume_get_first(void) { return volume_base; }
...
fstatus volume_set_label(struct volume_s *vol, const char *name,
                         uint8_t length) {
    // Make a directory object pointing to the root directory
    struct dir_s dir;
    dir.sector = vol->root_lba;
...
fstatus fat_dir_make(const char *path) { return FSTATUS_OK; }
...
/// has been hit.
fstatus fat_file_read(struct file_s *file, uint8_t *buffer, uint32_t count,
                      uint32_t *status) {
    *status = 0;
...
fstatus fat_file_write(struct file_s *file, const uint8_t *buffer,
                       uint32_t count) {
    return FSTATUS_OK;
}
