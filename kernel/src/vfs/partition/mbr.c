...
uint32_t read_mbr(const char *disk, struct vfs_partition *partitions,
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // For some reason, fdisk initialices attributes as 0, so we have to
        // check Validity by sector size. I'm guessing a zero size partition
        // must be bogus.
        if (mbr_header->partitions[i].number_of_sectors != 0x0) {
            add_part(partitions, mbr_header->partitions[i].lba_partition_start,
                     mbr_header->partitions[i].number_of_sectors, 1,
                     mbr_header->partitions[i].partition_type);
            printf("[MBR] Partition %d: LBA %d, size %d\n", i,
                   mbr_header->partitions[i].lba_partition_start,
                   mbr_header->partitions[i].number_of_sectors);
