...
uint32_t read_gpt(const char *disk, struct vfs_partition *partitions,
    ...
    struct gpt_header *gpt_header =
        (struct gpt_header *)malloc(sizeof(struct gpt_header));
    memset((uint8_t *)gpt_header, 0, sizeof(struct gpt_header));
    if (!disk_read(disk, (uint8_t *)gpt_header, 1, 1))
        return 0;
    ...
    for (uint32_t i = 0; i < gpt_header->partition_count; i += 4) {
        uint32_t lba = gpt_header->partition_table_lba +
                       ((i * gpt_header->partition_entry_size) >> 9);
        if (!disk_read(disk, (uint8_t *)(&gpt_entries[i]), lba, 1))
            return 0;
    }
    ...
    for (uint32_t i = 0; i < gpt_header->partition_count; i++) {
        ...
        sprintf(buffer, "%08lx-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
                ...
                gpt_entries[i].type_guid[8], gpt_entries[i].type_guid[9],
                gpt_entries[i].type_guid[10], gpt_entries[i].type_guid[11],
                gpt_entries[i].type_guid[12], gpt_entries[i].type_guid[13],
                gpt_entries[i].type_guid[14], gpt_entries[i].type_guid[15]);
    ...
    for (uint8_t i = 0; i < valid_partitions; i++) {
        add_part(partitions, gpt_entries[i].first_lba,
                 (gpt_entries[i].last_lba - gpt_entries[i].first_lba), 0, 0);
        printf("[GPT] Partition %d: LBA %d, size %d\n", i,
               gpt_entries[i].first_lba,
               (gpt_entries[i].last_lba - gpt_entries[i].first_lba));
    }
