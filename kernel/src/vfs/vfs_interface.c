...
int vfs_file_open(const char *path, int flags, int mode) {
    ...
    int res = -1;
    if (mount != 0) {
        res = mount->fst->file_open(mount->internal_index, native_path_buffer,
                                    flags, mode);
    }
...
uint64_t vfs_file_read(int fd, void *buffer, uint64_t size) {
    ...
    int res = -1;
    if (mount != 0) {
        res = mount->fst->file_read(mount->internal_index, fd, (char *)buffer,
                                    size);
    }
    free(native_path_buffer);
    free(path);
...
uint64_t vfs_file_write(int fd, void *buffer, uint64_t size) {
    ...
    int res = -1;
    if (mount != 0) {
        res = mount->fst->file_write(mount->internal_index, fd, (char *)buffer,
                                     size);
    }
    free(native_path_buffer);
    free(path);
...
int vfs_file_creat(const char *path, int mode) {
    ...
    int res = -1;
    if (mount != 0) {
        res = mount->fst->file_creat(mount->internal_index, native_path_buffer,
                                     mode);
    }
    free(native_path_buffer);
    return res;
...
int vfs_file_search(const char *name, char *path) {
    ...
    while (vfs_dir_read(fd, name_buffer, &name_len, &type) > 0) {
        ...
        if (type == 0x2 && strcmp(name_buffer, ".") != 0 &&
            strcmp(name_buffer, "..") != 0 &&
            strcmp(name_buffer, "lost+found") != 0) {
...
int vfs_dir_read(int fd, char *name, uint32_t *name_len, uint32_t *type) {
    ...
    int res = -1;
    if (mount != 0) {
        res = mount->fst->dir_read(mount->internal_index, fd, name, name_len,
                                   type);
    }
...
int vfs_mkdir(const char *path, int mode) {
    ...
    int res = -1;
    if (mount != 0) {
        res = mount->fst->dir_creat(mount->internal_index, native_path_buffer,
                                    mode);
    }
    free(native_path_buffer);
    return res;
...
int vfs_remove(const char *path, uint8_t force) {
    ...
    int res = -1;
    if (mount != 0) {
        res = mount->fst->prepare_remove(mount->internal_index,
                                         native_path_buffer);
        if (res == 0)
            return -2;
        if (is_safe_for_removing(native_path_buffer, force) == 0)
            return -1;
