...
#define __UNDEFINED_HANDLER                                                    \
    __asm__("cli");                                                            \
    (void)frame;                                                               \
    panic("Undefined interrupt handler");
...
void (*dynamic_interrupt_handlers[256])(struct cpu_context *ctx,
                                        uint8_t cpuid) = {0};
...
void pic_end_master() { outb(PIC1_COMMAND, PIC_EOI); }
...
static void interrupt_exception_handler(struct cpu_context *ctx,
                                        uint8_t cpu_id) {
...
struct idtdescentry *get_idt_gate(uint8_t entry_offset) {
    return (struct idtdescentry *)(idtr.offset + (entry_offset *
                                                  sizeof(struct idtdescentry)));
}

void set_idt_gate(uint64_t handler, uint8_t entry_offset, uint8_t type_attr,
                  uint8_t ist, uint16_t selector) {
    struct idtdescentry *interrupt =
        (struct idtdescentry *)(idtr.offset +
                                (entry_offset * sizeof(struct idtdescentry)));
...
void init_interrupts() {
    ...
    for (int i = 0; i < 256; i++) {
        set_idt_gate((uint64_t)interrupt_vector[i], i, IDT_TA_InterruptGate, 0,
                     get_kernel_code_selector());
    }
...
void global_interrupt_handler(struct cpu_context *ctx, uint8_t cpu_id) {
    __asm__("cli");
    void (*handler)(struct cpu_context *ctx, uint8_t cpu_id) =
        (void *)dynamic_interrupt_handlers[ctx->interrupt_number];
