...
// https://github.com/kot-org/Kot/blob/main/Sources/Kernel/Src/heap/heap.cpp
// Thank you Konect!!!
...
void initHeap(void *heapAddress, void *stackAddress, uint64_t pageCount) {
    ...
    globalHeap.mainSegment =
        (struct heap_segment_header *)((uint64_t)globalHeap.heapEnd +
                                       ((uint64_t)PAGESIZE -
                                        sizeof(struct heap_segment_header)));
    ...
    globalHeap.mainSegment->next =
        (struct heap_segment_header *)((uint64_t)globalHeap.mainSegment -
                                       (uint64_t)PAGESIZE +
                                       sizeof(struct heap_segment_header));

    globalHeap.mainSegment->next->free = 1;
    globalHeap.mainSegment->next->isStack = 0;
    globalHeap.mainSegment->next->length = (uint64_t)PAGESIZE -
                                           sizeof(struct heap_segment_header) -
                                           sizeof(struct heap_segment_header);
...
int heap_safeguard() { return ready; }
...
struct heap_segment_header *GetHeapSegmentHeader(void *address) {
    return (
        struct heap_segment_header *)(void *)((uint64_t)address -
                                              sizeof(
...
struct heap_segment_header *splitSegment(struct heap_segment_header *segment,
                                         uint64_t size) {
    if (segment->length <= size + sizeof(struct heap_segment_header))
        return 0;
    if (segment->signature != HEAP_SIGNATURE) {
        ready = 0;
        panic("Invalid heap segment signature split");
    }
    struct heap_segment_header *newSegment =
        (struct heap_segment_header *)(void *)((uint64_t)segment +
                                               segment->length - size);
...
void walk_heap() {
    struct heap_segment_header *currentSegment =
        (struct heap_segment_header *)globalHeap.mainSegment;
    while (1) {
        if (currentSegment->signature != HEAP_SIGNATURE) {
            printf("***************** HEAP SIGNATURE INVALID BELOW "
                   "*****************\n");
        }
        dump_segment(currentSegment);
        if (currentSegment->next == 0)
            break;
...
void *malloc(uint64_t size) {
    ...
    while (1) {
        if (currentSegment->signature != HEAP_SIGNATURE) {
            ready = 0;
            walk_heap();
            panic("Invalid heap segment signature malloc");
        }
        if (currentSegment->free) {
            if (currentSegment->length > sizeWithHeader) {
                ...
                globalHeap.usedSize +=
                    currentSegment->length + sizeof(struct heap_segment_header);
                globalHeap.freeSize -=
                    currentSegment->length + sizeof(struct heap_segment_header);
                return (void *)((uint64_t)currentSegment +
                                sizeof(struct heap_segment_header));
        ...
        if (currentSegment->next == 0)
            break;
        if (currentSegment->next == currentSegment) {
            ready = 0;
            panic("Current segment cycle\n");
        }
        currentSegment = currentSegment->next;
    }
...
void free(void *address) {
    ...
    struct heap_segment_header *header =
        (struct heap_segment_header *)((uint64_t)address -
                                       sizeof(struct heap_segment_header));
    if (header->signature != HEAP_SIGNATURE) {
        ready = 0;
        panic("Invalid free, signature");
    }
    if (header->free) {
        ready = 0;
        panic("Invalid free, double free");
    }
    header->free = 1;
...
void *realloc(void *buffer, uint64_t size) {
    ...
    uint64_t oldSize =
        GetHeapSegmentHeader(buffer)->length; // TODO: Check if this is correct
    if (size < oldSize)
        oldSize = size;
...
void expand_heap(uint64_t length) {
    ...
    for (uint64_t i = 0; i < pages; i++) {
        ...
        if (newPage == 0) {
            ready = 0;
            panic("Failed to expand heap");
        }
        globalHeap.heapEnd =
            (void *)((uint64_t)globalHeap.heapEnd - (uint64_t)PAGESIZE);
        map_current_memory(globalHeap.heapEnd, newPage);
    }

    struct heap_segment_header *newSegment =
        (struct heap_segment_header *)globalHeap.heapEnd;

    if (globalHeap.lastSegment != 0 && globalHeap.lastSegment->free &&
        globalHeap.lastSegment->last != 0) {
...
void *stackalloc(uint64_t length) {
    ...
    globalHeap.lastStack =
        (void *)((uint64_t)globalHeap.lastStack - (uint64_t)PAGESIZE);
