...
void print_pde(struct page_directory_entry *pde) {
    printf("p: %d w: %d u: %d wt: %d c: %d a: %d i3: %d s: %d i2: %d [pp: "
           "0x%x] r: %d i1: %d n: %d\n",
           pde->present, pde->writeable, pde->user_access, pde->write_through,
           pde->cache_disabled, pde->accessed, pde->ignored_3, pde->size,
           pde->ignored_2, pde->page_ppn, pde->reserved_1, pde->ignored_1,
           pde->execution_disabled);
}

void print_pte(struct page_table_entry *pte) {
    printf("p: %d w: %d u: %d wt: %d c: %d a: %d d: %d s: %d g: %d i2: %d [pp: "
           "0x%x] r: %d i1: %d n: %d\n",
           pte->present, pte->writeable, pte->user_access, pte->write_through,
           pte->cache_disabled, pte->accessed, pte->dirty, pte->size,
           pte->global, pte->ignored_2, pte->page_ppn, pte->reserved_1,
           pte->ignored_1, pte->execution_disabled);
}

uint64_t virtual_to_physical(struct page_directory *pml4,
                             void *virtual_memory) {
    ...
    uint64_t physical =
        ((uint64_t)pte.page_ppn << 12) | ((uint64_t)virtual_memory & 0xfff);
    return physical;
}
...
void debug_memory_map(struct page_directory *pml4, void *virtual_memory,
...
void init_paging() {
    ...
    if (linker_kstart != virtual_start) {
        printf("Crashing: KERNEL_START: %llx VIRT_ADDR: %p\n", linker_kstart,
               virtual_start);
        panic(
            "init_paging: kernel virtual address does not match KERNEL_START");
    }
}
...
void map_current_memory_size(void *virtual_memory, void *physical_memory,
                             uint64_t size) {
    struct page_directory *pml4 = get_pml4();
    map_memory_size(pml4, virtual_memory, physical_memory, size);
}

void map_memory_size(struct page_directory *pml4, void *virtual_memory,
                     void *physical_memory, uint64_t size) {
...
void map_memory(struct page_directory *pml4, void *virtual_memory,
                void *physical_memory) {
...
void set_page_perms(struct page_directory *pml4, void *address,
                    uint8_t permissions) {
    struct page_map_index map;
...
void *request_accessible_page_at(struct page_directory *pml4, void *vaddr,
                                 void *access_pointer) {
    void *result = request_page();
    if (result == NULL) {
        panic("ERROR: Could not allocate page for mapping\n");
...
void mprotect(struct page_directory *pml4, void *address, uint64_t size,
              uint8_t permissions) {
