...
#define ALIGN_ADDR(x)                                                          \
    (((PAGESIZE - 1) & (x)) ? ((x + PAGESIZE) & ~(PAGESIZE - 1)) : (x))
...
#define GET_PAGE_BIT(x)                                                        \
    ((memory->bitfield[(x) >> 3] & (1 << ((x) % 8))) >> ((x) % 8))
...
void free_page(void *addr) {
    ...
    uint64_t normaliced_address =
        (addr_uint - memory->first_available_page_addr);
...
void iterate_memory(void (*callback)(void *, uint64_t, uint64_t, uint64_t),
                    void *global_override) {
...
struct system_memory *test_get_memory() { return memory; }
...
int init_memory() {
    ...
    uint64_t last_available_adress =
        ALIGN_ADDR(biggest_avail_chunk.addr + biggest_avail_chunk.size);
    uint64_t available_pages =
        (last_available_adress - first_available_address) >> 12;
    uint64_t bitfield_entries = ((available_pages % 8) == 0)
                                    ? (available_pages >> 3)
                                    : (available_pages >> 3) + 1;

    if (biggest_avail_chunk.size <
        (bitfield_entries + sizeof(struct system_memory) + (3 << 12)))
    ...
    if (!IS_ALIGNED(memory) || !IS_ALIGNED(memory->bitfield) ||
        !IS_ALIGNED(memory->first_available_page_addr))
        panic("Memory alignment error!");

    uint64_t used_space =
        (bitfield_entries + sizeof(struct system_memory) + (3 << 12)) >> 12;
