...
void _tty_flush(struct tty *device) {
    ...
    device_write(device->outdev, len, 0,
                 (uint8_t *)(device->outb + device->outb_read));
...
// TODO: Maybe it is best for the line discipline to send itself and contain a
// field with the tty! idk...
...
void _tty_add_subscriber(struct tty *tty, void (*handler)(void *, uint8_t)) {
    ...
    struct tty_subscriber *new_subscriber =
        malloc(sizeof(struct tty_subscriber));
    if (new_subscriber == 0)
        return;
...
void tty_destroy(struct tty *tty) {
    ...
    if (tty->line_discipline)
        free(tty->line_discipline);
    tty->valid = 0;
}
...
int tty_init(char *inb, char *outb, int mode, int inbs, int outbs) {
    ...
    if (tty == 0)
        return -1;
    if (mode == TTY_MODE_SERIAL || mode == TTY_MODE_PTY)
        tty->mode = mode;
    else
        return -1;
    ...
    tty->line_discipline = line_discipline_create(
        LINE_DISCIPLINE_MODE_CANONICAL, LINE_DISCIPLINE_MODE_ECHO_ON,
        LD_DEFAULT_TABLE, 1024, tty, flush_cb, echo_cb);
    tty->israw = 0;
    tty->echo = 1;
    ...
    case TTY_MODE_SERIAL: {
        struct serial_ioctl_subscriptor sync_data = {.parent = tty,
                                                     .handler = tty_read_cb};
...
void _tty_write(struct tty *tty, char *buffer, int size) {
    ...
    for (int i = 0; i < size; i++) {
        char tbuffer[4];
        int translation =
            line_discipline_translate(tty->line_discipline, buffer[i], tbuffer);
...
void _tty_modes(struct tty *tty, int mode, int value) {
    ...
    case _TTY_MODE_RAW: {
        tty->israw = value;
        if (value)
            line_discipline_set_mode(tty->line_discipline,
                                     LINE_DISCIPLINE_MODE_RAW);
        else
            line_discipline_set_mode(tty->line_discipline,
