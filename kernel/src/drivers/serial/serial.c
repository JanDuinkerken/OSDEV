...
void SERIAL_OF_HANDLER(struct cpu_context *ctx, uint8_t cpuid) {
    ...
    if (interrupted_serial_device) {
        ...
        printf("[WARNIG] Serial overflow interrupt, but cant find device to "
               "flush\n");
    }
}

// TODO: This are not the correct handlers!
struct serial_device serial_devices[] = {{.port = DEFAULT_COM1_PORT,
                                          .irq = IRQ_COM1,
...
void add_subscriber(struct serial_device *device, uint8_t type, void *parent,
                    void *handler) {
...
void remove_subscriber(struct serial_device *device, uint8_t type, void *parent,
                       void *handler) {
    struct serial_subscriber *subscriber;
    if (type == SERIAL_READ) {
        subscriber = device->read_subscribers;
...
struct serial_device *get_serial_by_comm(int comm) {
    if (!initialized)
        return 0;
    if (comm < 0 || comm >= MAX_COM_DEVICES)
        return 0;
    if (!serial_devices[comm].valid)
        return 0;
...
int init_serial_comm(int port) {
    ...
    outb(port + 0, 0xAB); // Test serial chip (send byte 0xAE and check if
...
void init_serial(int inbs, int outbs) {
    ...
    for (int i = 0; i < MAX_COM_DEVICES; i++) {
        ...
        if (init_serial_comm(device->port)) {
            ...
            device->write_subscribers =
                malloc(sizeof(struct serial_subscriber));
...
void serial_read_event_add(int port, void *parent,
...
void serial_read_event_remove(int port, void *parent,
                              void (*handler)(void *parent, char c, int port)) {
    if (!initialized)
        return;
...
void serial_write_event_add(int port, void *parent,
                            void (*handler)(void *parent, char c, int port)) {
    if (!initialized)
        return;
...
void serial_write_event_remove(int port, void *parent,
                               void (*handler)(void *parent, char c,
                                               int port)) {
    if (!initialized)
        return;
