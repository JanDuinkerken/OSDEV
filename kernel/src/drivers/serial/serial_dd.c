...
uint64_t serial_dd_read_block(uint64_t port, uint64_t size, uint64_t skip,
                              uint8_t *buffer) {
...
uint64_t serial_dd_write_block(uint64_t port, uint64_t size, uint64_t skip,
                               uint8_t *buffer) {
...
uint64_t serial_dd_ioctl(uint64_t port, uint64_t op, void *data) {
    ...
    case SERIAL_SUBSCRIBE_READ: {
        struct serial_ioctl_subscriptor *subscriptor =
            (struct serial_ioctl_subscriptor *)data;
    ...
    case SERIAL_UNSUBSCRIBE_READ: {
        struct serial_ioctl_subscriptor *subscriptor =
            (struct serial_ioctl_subscriptor *)data;
        serial_read_event_remove(port, subscriptor->parent,
                                 subscriptor->handler);
        return 1;
    }
    ...
    case SERIAL_UNSUBSCRIBE_WRITE: {
        struct serial_ioctl_subscriptor *subscriptor =
            (struct serial_ioctl_subscriptor *)data;
        serial_write_event_remove(port, subscriptor->parent,
                                  subscriptor->handler);
        return 1;
    }
...
struct file_operations serial_fops = {.read = serial_dd_read_block,
