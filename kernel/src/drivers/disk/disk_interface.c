...
int get_disk_status(const char *drive) { return (device_search(drive) != 0); }
...
uint8_t disk_read(const char *disk, uint8_t *buffer, uint32_t lba,
                  uint32_t count) {
...
uint8_t disk_write(const char *disk, uint8_t *buffer, uint32_t lba,
                   uint32_t count) {
