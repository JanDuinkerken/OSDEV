...
int8_t find_cmd_slot(struct ahci_port *port) {
    uint32_t slots =
        (port->hba_port->sata_active | port->hba_port->command_issue);
...
uint8_t identify(uint8_t port_no) {
    ...
    struct hba_command_header *command_header =
        (struct hba_command_header
             *)(uint64_t)(port->hba_port->command_list_base |
                          (((uint64_t)port->hba_port->command_list_base_upper)
                           << 32));
    command_header += slot;
    ...
    struct hba_command_table *command_table =
        (struct hba_command_table
             *)(uint64_t)(command_header->command_table_base_address |
                          ((uint64_t)
                               command_header->command_table_base_address_upper
                           << 32));
    memset(command_table, 0,
           sizeof(struct hba_command_table) +
               (command_header->prdt_length - 1) *
                   sizeof(struct hba_prdt_entry));
    command_table->prdt_entry[0].data_base_address_upper =
        (uint32_t)((uint64_t)buffer >> 32);
    command_table->prdt_entry[0].data_base_address =
        (uint32_t)((uint64_t)buffer);
    command_table->prdt_entry[0].byte_count = 512 - 1;
    command_table->prdt_entry[0].interrupt_on_completion = 1;

    struct hba_command_fis *command_fis =
        (struct hba_command_fis *)command_table->command_fis;
    ...
    while (port->hba_port->task_file_data & (ATA_DEV_BUSY | ATA_DEV_DRQ) &&
           spin < 1000000) {
        spin++;
    };
    if (spin == 1000000) {
...
uint8_t write_atapi_port(uint8_t port_no, uint64_t sector,
                         uint32_t sector_count) {
    if ((uint64_t)abar == 0)
        return 0;
    uint32_t useless = port_no + sector + sector_count;
    useless += 1;
    panic("write_atapi_port not implemented");
    return 0;
}

uint8_t read_atapi_port(uint8_t port_no, uint64_t sector,
                        uint32_t sector_count) {
    ...
    command_header->command_fis_length =
        sizeof(struct hba_command_fis) / sizeof(uint32_t);
    command_header->write = 0;
...
uint8_t read_port(uint8_t port_no, uint64_t sector, uint32_t sector_count) {
    ...
    for (i = 0; i < command_header->prdt_length - 1; i++) {
        command_table->prdt_entry[i].data_base_address =
            (uint32_t)(uint64_t)buffer;
        command_table->prdt_entry[i].data_base_address_upper =
            (uint32_t)((uint64_t)buffer >> 32);
...
void port_stop_command(struct ahci_port *port) {
    ...
    while (1) {
        if (port->hba_port->command_status & HBA_PxCMD_FR)
            continue;
        if (port->hba_port->command_status & HBA_PxCMD_CR)
            continue;
        break;
    }
}
...
void configure_port(struct ahci_port *port) {
    ...
    port->hba_port->command_list_base_upper =
        (uint32_t)((uint64_t)new_base >> 32);
    ...
    port->hba_port->fis_base_address_upper =
        (uint32_t)((uint64_t)new_fis_base >> 32);
    memset(new_fis_base, 0, 256);

    struct hba_command_header *command_header =
        (struct hba_command_header
             *)((uint64_t)port->hba_port->command_list_base |
                ((uint64_t)port->hba_port->command_list_base_upper << 32));
    ;
    for (int i = 0; i < 32; i++) {
        ...
        command_header[i].command_table_base_address =
            (uint32_t)(uint64_t)address;
        command_header[i].command_table_base_address_upper =
...
enum port_type check_port_type(struct hba_port *port) {
    ...
    if (device_detection != 0x03)
        return PORT_TYPE_NONE;
    if (interface_power_management != 0x01)
        return PORT_TYPE_NONE;
...
void init_ahci(uint32_t bar5) {
    ...
    mprotect_current((void *)abar, 4096,
                     PAGE_CACHE_DISABLE | PAGE_WRITE_BIT | PAGE_USER_BIT |
                         PAGE_NX_BIT);
