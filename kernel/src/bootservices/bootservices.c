...
void generic_terminal_callback(struct limine_terminal *terminal, uint64_t type,
...
void generic_terminal_writer(const char *str, uint64_t length) {
    struct limine_terminal *terminal =
        CALL_SERVICE(TERMINAL)->terminals[terminal_writer];
...
char *get_bootloader_name() { return CALL_SERVICE(BOOTLOADER)->name; }

char *get_bootloader_version() { return CALL_SERVICE(BOOTLOADER)->version; }

uint64_t get_terminal_count() { return CALL_SERVICE(TERMINAL)->terminal_count; }

uint64_t get_current_terminal() { return terminal_writer; }

int64_t get_boot_time() { return CALL_SERVICE(TIME)->boot_time; }

uint64_t get_memory_map_entries() { return CALL_SERVICE(MEMMAP)->entry_count; }
...
uint64_t get_rsdp_address() { return (uint64_t)CALL_SERVICE(RSDP)->address; }
...
uint32_t get_smp_flags() { return CALL_SERVICE(SMP)->flags; }

uint32_t get_smp_bsp_lapic_id() { return CALL_SERVICE(SMP)->bsp_lapic_id; }

uint64_t get_smp_cpu_count() { return CALL_SERVICE(SMP)->cpu_count; }
